...
class RunSimulation:
    ...
    def run(self):
        ...
        X_train = torch.from_numpy(
            X_train_np).requires_grad_().to(torch.float32)
        U_train = torch.from_numpy(
            U_train_np).requires_grad_().to(torch.float32)
        X_border = torch.from_numpy(
            X_border_np).requires_grad_().to(torch.float32)
        X_border_test = (
            torch.from_numpy(X_border_test_np).requires_grad_().to(
                torch.float32)
        )
        ...
        X_pde = rectangle.generate_lhs(self.hyper_param["nb_points_pde"])

        # Data test loading
        torch.manual_seed(self.random_seed_test)
        np.random.seed(self.random_seed_test)
        X_test_pde = rectangle.generate_lhs(self.hyper_param["n_pde_test"])
        ...
        with open(self.folder_result + "/print.txt", "a") as f:
            model, optimizer, scheduler, loss, train_loss, test_loss, weights = (
                init_model(f, self.hyper_param,
                           self.device, self.folder_result)
            )
