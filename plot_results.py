...
def plot_flow(x, y, t, norme_vitesse_data, norme_vitesse_predict, name_file, fps=7, title='Norme vitesse'):
    ...
    # Créer l'animation
    ani = FuncAnimation(fig, update, frames=len(set(t)),
...
def plot_loss_decompose(file_save, file, title_graph):
    ...
    plt.plot(epochs, test_border, label='Loss border test',
...
def plot_points(X_train, X_border, mean_std, param_adim, file_save):
    ...
    x = (X_train[:, 0][masque]*mean_std['x_std'] +
         mean_std['x_mean'])*param_adim['L']
    y = (X_train[:, 1][masque]*mean_std['y_std'] +
...
def plot_cl(X_full, U_full, model, param_adim, mean_std, file_save, r=0.025/2):
    ...
    masque = ((X_full[:, 0] > x_min) & (X_full[:, 0] < x_max)
              & (X_full[:, 1] > y_min) & (X_full[:, 1] < y_max))
    X_red = X_full[masque]
    U_red = U_full[masque]
    ...
    def P_base(theta, t_ad):
        ...
        P_dim = ((U_red[indice_proche][2] * mean_std['p_std'] +
                 mean_std['p_mean']))*((param_adim['V']**2)*param_adim['rho'])
        return P_dim
    ...
    time_1 = (np.unique(X_red[:, 2])*mean_std['t_std'] +
              mean_std['t_mean'])*(param_adim['L']/param_adim['V'])
    time_dim = (time * mean_std['t_std'] +
                mean_std['t_mean'])*(param_adim['L']/param_adim['V'])
...
def plot_results(
    ...
    checkpoint = torch.load('results/'+file_name+'/'+epoch +
    ...
    U_pred = model(X_pred)
    model.load_state_dict(checkpoint['model_state_dict'])
    x_norm_pred, y_norm_pred, t_norm_pred = X_pred.detach().numpy(
    )[:, 0], X_pred.detach().numpy()[:, 1], X_pred.detach().numpy()[:, 2]
    u_norm_pred, v_norm_pred, p_norm_pred = U_pred.detach().numpy(
    )[:, 0], U_pred.detach().numpy()[:, 1], U_pred.detach().numpy()[:, 2]
    x_pred = (x_norm_pred*mean_std['x_std'] +
              mean_std['x_mean'])*param_adim['L']
    y_pred = (y_norm_pred*mean_std['y_std'] +
              mean_std['y_mean'])*param_adim['L']
    t_pred = (t_norm_pred*mean_std['t_std'] +
              mean_std['t_mean'])*(param_adim['L']/param_adim['V'])
    ...
    y_data = (X_full[:, 1]*mean_std['y_std'] +
              mean_std['y_mean'])*param_adim['L']
    t_data = (X_full[:, 2]*mean_std['t_std'] +
              mean_std['t_mean'])*(param_adim['L']/param_adim['V'])
    ...
    # On plot les vidéos
    plot_flow(x_data, y_data, t_data, p_data, p_pred, name_file="results/" +
              '/'+file_name+'/'+'result_pression.gif', title='Pression', fps=5)
    plot_flow(x_data, y_data, t_data, norme_vitesse_data, norme_vitesse_pred,
              name_file="results/"+'/'+file_name+'/'+'result_vitesse.gif', title='Vitesse', fps=5)
    print("OK les vidéos")

    # On plot la loss
    plot_loss(file_save="results/"+file_name, file="results/" +
              file_name+'/'+epoch, title_graph=title_loss)
    print("OK le plot de la loss")

    # On plot la loss decompose
    plot_loss_decompose(file_save="results/"+file_name, file="results/" +
                        file_name+'/'+epoch, title_graph='loss_decompose')

    # On plot les points
    plot_points(X_train, X_border, mean_std, param_adim,
                file_save="results/"+file_name+'/points_coloc')
    print('OK pour les points de coloc')

    # On plot le Cl
    plot_cl(X_full, U_full, model, param_adim, mean_std,
